...
public class FirebaseManager {
    ...
    public void registerUser(Users user, Activity activity) {
        ...
        getmAuth().createUserWithEmailAndPassword(user.getEmail(), user.getPass()).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                FirebaseUser fbUser = mAuth.getCurrentUser();

                if (fbUser != null) {
                    // String userId = user.getUid();
                    DatabaseReference currentUserRef = usersRef.child(user.getUsername());
                    DatabaseReference userInfoRef = currentUserRef.child("user info");
                    userInfoRef.child("username").setValue(user.getUsername());
                    userInfoRef.child("email").setValue(user.getEmail());
